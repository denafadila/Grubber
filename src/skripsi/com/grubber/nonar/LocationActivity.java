...
/**
 * This the app's main Activity. It provides buttons for requesting the various features of the app,
 * displays the current location, the current address, and the status of the location client and
 * updating services.
 * 
 ...
 * the Activity. {@link #stopUpdates} cancels previous periodic update requests.
 * 
...
public class LocationActivity extends FragmentActivity implements LocationListener,
  ...
  List<Restaurant> mResult = new ArrayList<Restaurant>();
  List<Restaurant> sortedRest = new ArrayList<Restaurant>();
  ...
  /*
   * Note if updates have been turned on. Starts out as "false"; is set to "true" in the method
   * handleRequestSuccess of LocationUpdateReceiver.
   */
  ...
  /*
   * Called when the Activity is going into the background. Parts of the UI may be visible, but the
  ...
  /**
   * Invoked by the "Get Address" button. Get the address of the current location, using reverse
   * geocoding. This only works if a geocoding service is available.
   * 
   * @param v
   *          The view object associated with this method, in this case a Button.
   */
  ...
  /**
   * An AsyncTask that calls getFromLocation() in the background. The class uses the following
   * generic types: Location - A {@link android.location.Location} object containing the current
   * location, passed as the input parameter to doInBackground() Void - indicates that progress
   * units are not used by this subclass String - An address passed to onPostExecute()
   */
  protected class GetAddressTask extends AsyncTask<Location, Void, String> {
    ...
    protected String doInBackground(Location... params) {
       ...
       * android.location.Geocoder, but other geocoders that conform to address standards can also
       * be used.
       */
      ...
      try {
        ...
        /*
         * Call the synchronous getFromLocation() method with the latitude and longitude of the
         * current location. Return at most 1 address.
         */
    ...
    /**
     * A method that's called once doInBackground() completes. Set the text of the UI element that
     * displays the address. This method runs on the UI thread.
     */
    @Override
  ...
  private void initilizeMap() {
    if (googleMap == null) {
      ...
      googleMap.moveCamera(CameraUpdateFactory.newLatLng(new LatLng(currentLatitude,
          currentLongitude)));
  ...
  private void addMarker() {
    ...
    googleMap.clear();
    // create marker for nearby rest
    for (int i = 0; i < sortedRest.size(); i++) {
      // adding marker
      markerMap.put(
          googleMap.addMarker(new MarkerOptions()
              .position(new LatLng(sortedRest.get(i).getLat(), sortedRest.get(i).getLong()))
              .title(sortedRest.get(i).getName()).snippet(sortedRest.get(i).getCity())
              .icon(BitmapDescriptorFactory.fromResource(R.drawable.pinpoint))), sortedRest.get(i));
      ...
      googleMap.setOnInfoWindowClickListener(new OnInfoWindowClickListener() {
        ...
        public void onInfoWindowClick(Marker arg0) {
          ...
          if (markerMap.containsKey(arg0)) {
            Intent intent = new Intent(LocationActivity.this, RestaurantProfileActivity.class);
        ...
        if (mResult != null) {
          ...
          for (int i = 0; i < mResult.size(); i++) {
            ...
            if (countDist(mResult.get(i).getLat(), mResult.get(i).getLong()) < 2) {
              // moving nearby rest to sortedRest var
              sortedRest.add(size++, mResult.get(i));
            }
          }
        }
